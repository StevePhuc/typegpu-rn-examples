...
export function createNodeTransformState(
  nodeCount: number,
): NodeTransformState {
...
export function sampleAnimationInto(
  ...
  for (const channel of animation.channels) {
    ...
    let keyframeIndex = 0;
    while (
      keyframeIndex < times.length - 2 &&
      loopedTime >= times[keyframeIndex + 1]
    ) {
      keyframeIndex++;
    }
    ...
    const alpha =
      endTime > startTime
        ? Math.max(
            0,
            Math.min(1, (loopedTime - startTime) / (endTime - startTime)),
          )
        : 0;
